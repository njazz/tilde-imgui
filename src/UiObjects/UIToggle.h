...
public:
    ...
    virtual void drawObjectContents() override
    {
        ...
        if (_value){
        draw_list->AddLine(ImVec2(getX(), getY()), ImVec2(getX() + width, getY() + height), IM_COL32(0, 192, 255, 255), width / 20);
        draw_list->AddLine(ImVec2(getX() + width, getY()), ImVec2(getX(), getY() + height), IM_COL32(0, 192, 255, 255), width / 20);
        }
